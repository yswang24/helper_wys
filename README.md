...
其余操作走界面/托盘：主窗口显隐点 Dock / 托盘图标；穿透↔输入走托盘；清空走浮层或主窗口的「清空」按钮；退出走托盘菜单「退出」。
...
浏览器类判题平台（HackerRank/Coderpad/牛客）盯 `window.blur` / `document.visibilitychange` 来判「切屏」。macOS + Electron 28 上「浮层能打字」与「永不激活本 app」无法靠单窗口同时满足，故拆成**两种模式**，**只用托盘切换**（头部徽标只是当前模式指示器、不可点）：
...
浮层对截屏隐身（`setContentProtection`），不能靠录屏。用前台浏览器直接听事件：控制台粘贴 ↓，保持浏览器在前台，通过托盘切到输入模式、点输入框打字，看有没有蹦字。
