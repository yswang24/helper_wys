...
| fn⌥ | 开启 / 关闭回答滚动模式；开启后使用 `↑` / `↓` 翻阅 |
| fn⌘ | 显示 / 隐藏浮层 |
| ⌥⌘X | 浮层 穿透 / 输入 模式切换（见「焦点」一节） |
| ⌥⌘Z | 截图解题（框选区域）；选区打开时再次按下可取消 |

其余操作走界面/托盘：主窗口显隐点 Dock / 托盘图标；穿透↔输入也可走托盘；清空走浮层或主窗口的「清空」按钮；退出走托盘菜单「退出」。

快捷键被别的 app 占用或原生快捷键组件无法启动时，主窗口顶部会出现黄色提示。

回答滚动模式仅在 macOS 提供，使用规则如下：

- 第一次按 `fn⌥` 会原子注册全局 `↑` / `↓` 并开启模式。浮层头部出现 `↕ 回答滚动` 时表示模式已开启。
- 开启后直接按 `↑` / `↓` 即可翻阅；每次方向键都会把 30 秒无操作倒计时重新计时。滚动到顶部或底部后继续按不会越界。
- 再按一次 `fn⌥` 会关闭模式。连续 30 秒没有方向键操作也会自动关闭。
- 模式开启期间，全局 `↑` / `↓` 由 Helper 接管，**前台 app 不会收到这两个方向键**；用完请关闭模式，或等待 30 秒自动关闭。
- 流式回答中向上回看只会暂停视口自动追底，模型仍会继续生成，浮层会显示 `↕ 回看中 · 生成继续`。关闭或超时退出滚动模式后会立即回到实时输出末尾。
- 若任一全局方向键被其他 app 占用，Helper 会回滚另一方向键并且不进入模式。
...
| **屏幕录制** | 框选截图（⌥⌘Z）和全屏直发（fn⇧） | 系统设置 → 隐私与安全性 → 屏幕录制 |
...
浮层对截屏隐身（`setContentProtection`），不能靠录屏。用前台浏览器直接听事件：控制台粘贴 ↓，保持浏览器在前台，再用 `⌥⌘X` 将浮层切到输入模式、点输入框打字，看有没有蹦字。
...
`⌥⌘Z` 唤出选区遮罩 → 拖拽框选题目区域 → 截图交给视觉模型。两种模式（设置页「截图解题模式」切换）：
...
> 取消选区：再按一次 `⌥⌘Z`（选区是非激活面板、收不到键盘，所以没有 Esc/Enter 快捷键）。

`⌥⌘Z` 走 macOS 原生区域抓取（`screencapture -R`），只截选区、不再整屏抓取，更快；编码用 JPEG 进一步压上传体积。
