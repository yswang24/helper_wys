...
- **全局翻阅回答**：按 `fn↑` / `fn↓` 平滑滚动浮层回答，穿透模式下也无需切换焦点。
...
`fn↑` / `fn↓` 在浮层未获取焦点、处于穿透模式时同样有效；长按会连续滚动，滚动到顶部或
底部后继续按不会越界。

> macOS 会同时把 `fn↑` / `fn↓` 解释为 Page Up / Page Down。为保持零额外系统权限，
> Helper 当前只观察、不吞掉按键，因此前台网页也可能同步滚动。若要只滚 Helper，需要另行
> 启用带辅助功能 / 输入监控权限的按键拦截方案。

---

## 开发与构建

macOS 需安装 Xcode Command Line Tools，用于编译 `fn⇧` / `fn↑` / `fn↓` 的 Universal 原生快捷键组件。
...
| 模式 | 行为 | 焦点 |
|---|---|---|
| **穿透 🔒（默认）** | 点击穿到下层，浮层只读展示答案；可用 `fn↑` / `fn↓` 翻阅 | `focusable:false` + `showInactive()` + `setIgnoreMouseEvents(true)`，不激活本 app、不抢焦点 |
...
应用只轮询 Fn/Shift 与方向键的硬件状态，不读取字符输入，也不需要辅助功能或输入监控权限。
