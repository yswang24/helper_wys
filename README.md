...
- **全局翻阅回答**：按 `fn⌥` 临时开启回答滚动模式，随后直接用 `↑` / `↓` 平滑翻页，穿透模式下也无需切换焦点。
...
回答滚动模式会接管裸 `↑` / `↓`，每次方向键都会重置 30 秒无操作计时；手动或超时关闭时，都会释放两个方向键。

其余操作走界面/应用菜单：普通点击 Dock 图标显示主窗口；右键 Dock 图标可显示/隐藏浮层、切换穿透↔输入或退出；Windows/Linux 继续使用托盘菜单；清空走浮层或主窗口的「清空」按钮。macOS 不创建菜单栏状态图标。

快捷键被别的 app 占用或原生快捷键组件无法启动时，主窗口顶部会出现黄色提示。

回答滚动模式仅在 macOS 提供，使用规则如下：

- 按 `fn⌥` 会原子注册全局 `↑` / `↓` 并开启模式；开启时不会立即滚动。浮层头部出现 `↕ 回答滚动` 时表示模式已开启。
- 开启后直接按 `↑` / `↓` 即可翻阅；每次方向键都会把 30 秒无操作倒计时重新计时。滚动到顶部或底部后继续按不会越界。
- 再按 `fn⌥` 会关闭模式；这次按键不会滚动。连续 30 秒没有方向键操作也会自动关闭。
- 模式开启期间，全局 `↑` / `↓` 由 Helper 接管，**前台 app 不会收到这两个方向键**；用完请关闭模式，或等待 30 秒自动关闭。
- 若任一全局方向键被其他 app 占用，Helper 会回滚另一方向键，不进入模式。

---

## 开发与构建

macOS 需安装 Xcode Command Line Tools，用于编译四种 `fn` 修饰键的 Universal 原生快捷键组件。
...
| 权限 | 用途 | 在哪开 |
|---|---|---|
| **麦克风** | 语音转写（fn⌃） | 系统设置 → 隐私与安全性 → 麦克风 |
| **屏幕录制** | 区域截图和全屏直发（fn⇧） | 系统设置 → 隐私与安全性 → 屏幕录制 |
...
3. 应用语音页点「**授权/刷新**」授麦克风权限 → 设备下拉选 **BlackHole** → fn⌃ 开始。
...
浏览器类判题平台（HackerRank/Coderpad/牛客）盯 `window.blur` / `document.visibilitychange` 来判「切屏」。macOS + Electron 28 上「浮层能打字」与「永不激活本 app」无法靠单窗口同时满足，故拆成**两种模式**，通过 `⌥⌘X` 或 Dock 右键菜单切换（头部徽标只是当前模式指示器、不可点）：
...
浮层对截屏隐身（`setContentProtection`），不能靠录屏。用前台浏览器直接听事件：控制台粘贴 ↓，保持浏览器在前台，通过 `⌥⌘X` 切到输入模式、点输入框打字，看有没有蹦字。
...
区域截图遵循设置页「截图解题模式」的两种模式：

- **直接解答（默认）**：截图直接喂给视觉模型，一次调用**流式输出答案**到浮层，最快。
- **先识别可编辑**：先 OCR 出题目文字，确认 / 纠错后再发给问答模型——适合视觉模型偶尔看错代码的场景。

`fn⇧` 始终是全屏直接解答路径：截取按键时鼠标所在的整块显示器、显示隐身浮层，并直接发送给
视觉模型流式解答，不进入 OCR 审阅流程；截图尚未完成时重复触发不会并行抓取。
原生组件只轮询 Fn 与四种修饰键状态，不读取字符输入，也无需应用主动请求辅助功能或输入监控权限。
