...
describe('useAsrCapture', () => {
  it('first Fn+Control starts a recording', async () => {
  ...
  it('second Fn+Control stops, transcribes, and appends to the draft', async () => {
    const { result } = renderHook(() => useAsrCapture(false))
    await act(async () => {
      await toggle()
    }) // start
  ...
  it('8s guard force-resets transcribing if onstop never fires (Fn+Control not stranded)', async () => {
    vi.useFakeTimers()
    const { result } = renderHook(() => useAsrCapture(false))
    await act(async () => {
      await toggle()
